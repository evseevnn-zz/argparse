...
class ArgumentParser extends _ActionsContainer
{
  ...
  _parse_known_args(arg_strings, namespace) {
        ...
        if (explicit_arg) {
          ...
          if (arg_count === 0 && !chars.includes(option_string[1])) {
            ...
            if (this._option_string_actions.has(option_string)) {
              action = this._option_string_actions.get(option_string);
    ...
    console.log('tes'); process.exit();
    let positionals = this._get_positional_actions();
console.log(positionals); process.exit();
  ...
  _parse_optional(arg_string) {
    ...
    if (this._option_string_actions.has(arg_string)) {
      let action = this._option_string_actions.get(arg_string);
    ...
    if (arg_string.includes('=')) {
      ...
      if (this._option_string_actions.has(option_string)) {
        let action = this._option_string_actions.get(option_string);
    ...
    if (this._negative_number_matcher.test(arg_string) && !this._has_negative_number_optionals) {
      return null;
    }
  ...
  print_usage(file = null, defaultStream = process.stdout) {
    ...
    stream.write(this.format_usage());
  }

  print_help(file = null, defaultStream = process.stdout) {
    let stream = file ? fs.createWriteStream(file) : defaultStream;
    stream.write(this.format_help());
  }
